 ...
 *
 * @file bsp_key_queue_value.c
 *
 * @par dependencies
 * - bsp_key_config.h
 * - bsp_key_queue_value.h
 *
...
/* Compile this implementation only when queue-by-value transfer is selected. */
...
void key_task_entry(void *argument)
{
    ...
    for (;;)
    {
        /* Step 1: Wait for an edge event from either physical key. */
        ...
        if (edge_event.key_id >= KEY_ID_COUNT)
        {
            printf("[E][KEY] Invalid key ID: %d.\r\n",
                   (int)edge_event.key_id);
            continue;
        }

        p_runtime = &key_runtime[edge_event.key_id];
        key_result = KEY_EVENT_NONE;
        press_complete = false;

        /* Step 3: Process the independent state machine of the source key. */
        switch (p_runtime->fsm_state)
        {
            case KEY_FSM_WAIT_PRESS:
            {
                if (KEY_EDGE_FALLING == edge_event.edge_type)
                {
                    p_runtime->press_tick   = edge_event.edge_tick;
            ...
            {
                if (KEY_EDGE_RISING == edge_event.edge_type)
                {
                    p_runtime->release_tick = edge_event.edge_tick;
                ...
                else if (KEY_EDGE_FALLING == edge_event.edge_type)
                {
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    /* Step 5: Send the complete raw edge event value when the queue is ready. */
    if (NULL != s_key_edge_queue)
    {
